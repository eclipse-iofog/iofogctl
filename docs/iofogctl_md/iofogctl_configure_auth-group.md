...
```
iofogctl configure auth-group NAME [flags]
```

### Examples

```
iofogctl configure auth-group secops --name platform-ops -n NAMESPACE
iofogctl configure auth-group viewer --mfa-required=true -n NAMESPACE
```
