...
func assertGoldenAgentConfig(t *testing.T, cfg *AgentConfiguration) {
	...
	require.NotNil(t, cfg.RouterConfig.RouterMode)
	require.Equal(t, "edge", *cfg.RouterConfig.RouterMode)
	require.NotNil(t, cfg.RouterConfig.MessagingPort)
	require.Equal(t, 5671, *cfg.RouterConfig.MessagingPort)
	require.NotNil(t, cfg.NatsConfig.NatsMode)
	require.Equal(t, "leaf", *cfg.NatsConfig.NatsMode)
...
func TestGoldenUnmarshalRemoteAgentPackageRegistry(t *testing.T) {
	...
	require.Equal(t, "1.0.0-rc.3", agent.Package.Version)
	require.Equal(t, "ghcr.io/datasance/edgelet:1.0.0-rc.3", agent.Package.Container.Image)
	...
	require.Equal(t, []string{"1.0.0-rc.3"}, agent.Scripts.Install.Args)
