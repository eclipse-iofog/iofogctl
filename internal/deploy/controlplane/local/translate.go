...
// TranslateOptions configures LocalControlPlane → edgelet manifest translation.
...
func NeedsPrivateEdgeletRegistry(cp *resource.LocalControlPlane) bool {
...
// Private registries use the ID from edgelet registry ls; otherwise online=1, airgap=2.
func ResolveEdgeletRegistryID(cp *resource.LocalControlPlane, privateRegistryID *int) *int {
...
// TranslateLocalControlPlane maps CLI LocalControlPlane to edgelet Registry (optional) and ControlPlane YAML.
func TranslateLocalControlPlane(cp *resource.LocalControlPlane, opts TranslateOptions) (TranslateResult, error) {
	...
	manifest := translateEdgeletControlPlane(cp, tOpts)
...
func TranslateEdgeletControlPlaneManifest(cp *resource.LocalControlPlane, opts TranslateOptions) edgeletControlPlaneManifest {
	return translateEdgeletControlPlane(cp, opts.mergeDefaults(opts.Namespace))
}

// TranslateEdgeletRegistryManifest returns the edgelet Registry manifest struct (test helper).
//
//nolint:revive // test helper intentionally returns package-private manifest type
func TranslateEdgeletRegistryManifest(cp *resource.LocalControlPlane) (edgeletRegistryManifest, error) {
	if !NeedsPrivateEdgeletRegistry(cp) {
		return edgeletRegistryManifest{}, util.NewError("Local Control Plane does not require a private edgelet registry")
	}
	return translateEdgeletRegistry(cp)
}

func translateEdgeletRegistry(cp *resource.LocalControlPlane) (edgeletRegistryManifest, error) {
	pkg := cp.Controller.Package
...
func translateEdgeletControlPlane(cp *resource.LocalControlPlane, opts translateOptions) edgeletControlPlaneManifest {
	...
	if sys := systemMicroservicesToEdgelet(cp.SystemMicroservices, opts); sys != nil {
		spec.SystemMicroservices = sys
	}
	if nats := natsToEdgelet(cp.Nats); nats != nil {
		spec.Nats = nats
	}
	if tls := tlsToEdgelet(cp.TLS); tls != nil {
		spec.TLS = tls
	}
...
func controllerImage(cp *resource.LocalControlPlane, defaultImage string) string {
...
func resolvePublicURL(cp *resource.LocalControlPlane) string {
...
func consoleToEdgelet(ctrl resource.LocalControllerSpec) *edgeletConsoleSpec {
...
func authToEdgelet(a resource.Auth) edgeletAuthSpec {
	out := edgeletAuthSpec{
		Mode:                      a.Mode,
...
func databaseToEdgelet(db resource.Database) *edgeletDatabaseSpec {
	if db.Provider == "" {
		return nil
	}
...
func eventsToEdgelet(ev resource.Events) *edgeletEventsSpec {
...
func natsToEdgelet(n *resource.NatsEnabledConfig) *edgeletNatsSpec {
	if n == nil {
		return nil
	}
...
func tlsToEdgelet(t *resource.ControlPlaneTLS) *edgeletTLSSpec {
...
func vaultToEdgelet(v *resource.VaultSpec) *edgeletVaultSpec {
	if v == nil {
		return nil
	}
