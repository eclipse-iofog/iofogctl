package deploymicroservicetemplate
...
func (exe *remoteExecutor) Execute() error {
	util.SpinStart(fmt.Sprintf("Deploying Microservice Template %s", exe.GetName()))
	...
	if len(controlPlane.GetControllers()) == 0 {
		return util.NewInputError("This namespace does not have a Controller. You must first deploy a Controller before deploying Microservice Templates")
	}
	...
	return apps.DeployMicroserviceTemplate(controller, baseURL, exe.template, exe.name, apps.WithAPIVersion(util.GetCliApiVersion()))
}
